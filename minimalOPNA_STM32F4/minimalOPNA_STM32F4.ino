...
#define YM_IC   PB1 // YM2608 initial clear, used as hardware reset. ACTIVE LOW

#define YM_RD   PB0// YM2608 read *from* synth. basically, unused here, because i pulled it to VCC. ACTIVE LOW
...
void setupMCO() {
  // enabling PORT A clocking
  RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;

  // setting up PA8 in Alternate Function (AF0 - MCO1 in STM32F4)
  // clearing MODER bits for PA8 (bits 16 and 17) and setting as '10' (Alternate function mode)
  GPIOA->MODER = (GPIOA->MODER & ~(3UL << (8 * 2))) | (2UL << (8 * 2));
  
  // highest speed for PA8 (Very High Speed)
  GPIOA->OSPEEDR |= (3UL << (8 * 2));
  
  // AFRH (Alternate Function High) for PA8. AF0 = 0000.
  GPIOA->AFR[1] = (GPIOA->AFR[1] & ~(0xFUL << 0)); 

  // 3. setting up internal generator MCO1
  // in STM32F4 internal RC generator HSI works at 16 МГц!
  // bits 22:21 = source select (00 = HSI). 
  // bits 26:24 = MCO1PRE divider (100 = division by 2). result: 16 / 2 = 8 mhz!
  RCC->CFGR = (RCC->CFGR & ~((3UL << 21) | (7UL << 24))) | ((0UL << 21) | (4UL << 24));
}
...
void writeYM(uint8_t port, uint8_t reg, uint8_t data) {
  ...
  digitalWrite(YM_A1, port); // choosing port: 1 - FM1-FM3 + SSG + RSS, 0 - FM4-FM6 + ADPCM. YM2608 is YM2203(OPN) compatible, remember that?
  ...
  digitalWrite(YM_WR, LOW);  // making it WRITE data
...
void checkButtons() {
  ...
  if (ok == LOW && btn_ok_last == HIGH) { 
    if (currentState == STATE_BROWSER) action_menu = true; // if in menu - plays selected file
      ...
      isPaused = !isPaused;                             // if in player - play/pause
      delay(50); 
      if (!isPaused) {
        track_sync_timer = micros();      // debounce delay compensation
        lastDrawTime = millis();
      }
    }
  }
...
void drawPlayer() {
  ...
  if (!isS98) { // if we are playing the vgm file
...
void playSelectedFile() {
  ...
  while(file.openNext(&dir, O_READ)){
    if(!file.isHidden() && !file.isDir()){
      char name[32]; file.getName(name, sizeof(name));
      if(strstr(name, ".vgm")||strstr(name, ".VGM")||strstr(name, ".s98")||strstr(name, ".S98")){
        if(c==fileIndex){ strcpy(currentFilename, name); file.close(); break; } // saving the name of the file
        c++;
      }
    }
    file.close();
...
//          VGm parser
      ...
      switch (cmd) {
        ...
        case 0x61: { 
            ...
            syncWaitUs(( (uint32_t)s * 10000 ) / 441);              // 1 sample @ 44.1кГц ≈ 22.6 us (evading any floating point operations to preserve precision, syncWaitUs will compesate. syncWaitUs <3)
          } break;
        ...
        // 0x67 - ADPCM samples
        case 0x67: { 
            uint32_t skip_start_time = micros(); //timestamp for compensating SD read latencies when jumping through ADPCM block, which is currently unused
            ...
            track_sync_timer += (micros() - skip_start_time); //kinda "forgiving" that jump time, so tempo wont float away.
          } break;                                            //yes, i have no idea either, wtf is even going on
